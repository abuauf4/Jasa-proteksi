...
import { Shield, Plane, PawPrint, Zap, UserCheck, ZoomIn } from "lucide-react";
...
const categoryKeys = ["all", "vehicle", "travel", "pet", "personal"];
    ...
    <SectionWrapper id="galeri" className="bg-[#0D0D0D]" dark>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map((item, i) => (
            <AnimatedSection key={item.titleKey} delay={i * 0.08} duration={0.5}>
              <div
                className="relative group cursor-pointer overflow-hidden rounded-xl hover:scale-[1.01] transition-transform duration-500"
                style={{ height: "280px" }}
              >
              {/* Gradient Background */}
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-white font-medium  text-lg group-hover:text-[#2E7D6F] transition-colors duration-500">{t(`gallery.items.${item.titleKey}`)}</p>
